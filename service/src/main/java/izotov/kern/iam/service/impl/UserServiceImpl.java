...
import izotov.kern.iam.excaption.PasswordRequiredException;
import izotov.kern.iam.excaption.UserAlreadyExistsException;
import izotov.kern.iam.excaption.UserNotFoundException;
...
public class UserServiceImpl implements UserService, ReactiveUserDetailsService, ReactiveUserDetailsPasswordService {
    ...
    public Mono<Boolean> exists(String username) {
        return findByUserName(username)
                .thenReturn(true)
                .onErrorReturn(UserNotFoundException.class, false); // UserNotFoundException
    }

    @Override
    @PreAuthorize("hasRole('ADMIN')")
    public Flux<PageableUser> findPageableUsers(Pageable pageable) {
        return userRepository.findUsers(Expressions.asBoolean(true), pageable.getOffset(), pageable.getPageSize())
                .map(user -> new PageableUser(user.username()));
    }

    @Override
    public Mono<UserCreated> newUser(@NonNull CreateUser user) {
    ...
    public Mono<UserDetails> findByUsername(String username) {
        return findByUserName(username)
                .zipWhen(user -> userRoleService.findUserRoles(user.id()))
                .map(tuple -> {
